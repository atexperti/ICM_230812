...
public class UserBoImpl implements IUserBo{
	...
	public UserMaster getUser(int id) {
		UserMaster = userDao.getUser(id);
		if(UserMaster != null && UserMaster.getStatus() == 1)
			return UserMaster;
		else
			return null;
	}
	...
	public UserMaster getUser(String uname, String pwd) {
		try{
			...
			if(!custList.isEmpty()){
				...
				if(UserMaster.getUserId() != 0 && UserMaster.getStatus() == 1){
					return UserMaster;
				}
			}
		}catch(Throwable tr){
	...
	@Override
	public void updateUser(UserMaster cust) {
